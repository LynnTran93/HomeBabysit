...
public class BabysitterProfileActivity extends AppCompatActivity {
   ...
   private void loadBabysitterData(String email) {
    ...
    if (cursor != null && cursor.moveToFirst()) {
        ...
        // Populate the fields if columns are valid
        ...
        if (availabilityIndex >= 0) {
            String availability = cursor.getString(availabilityIndex);
            String availabilityText = availability.equals("1") ? "available" : "unavailable";
            int spinnerPosition = ((ArrayAdapter) availabilitySpinner.getAdapter()).getPosition(availabilityText);
            availabilitySpinner.setSelection(spinnerPosition);
        }

        if (idIndex >= 0) {
            int babysitterId = cursor.getInt(idIndex);
